...
### c++ 字符串转int
atoi的参数是char*类型，c_str()包含在头文件<string>里
```
string s = "12"; 
int a = atoi(s.c_str());
```
...
2.项目
3.两个字符串的最长公共子序列
4.TCP里的TIME_WAIT有什么作用
5.链表倒数第k个节点
6.select和epoll
7.信号 (我说是操作系统里的信号量吗，他说不是)
8.一个文件里面的数排序，文件很大，内存一次加载不完
9.快排怎么改进
10.hash表的一致性
11.操作系统锁的类型
```
...
### 美团一面(牛客视频面)
```
1.http和https的区别，http的状态字，说一下tcp的四次挥手，拥塞控制算法
2.死锁的条件，怎么避免死锁，数据库怎么避免死锁
3.写一个查询语句，返回平均分大于80的学生姓名和平均分
4.数据库 inner join，left join
5.数据库索引什么情况下无效

6.写一个二分查找的非递归实现
7.写一个中序遍历的非递归实现
8.智力题，向北10km，向东10km，向南10km，回到原点，问有多少个这样的点
```
